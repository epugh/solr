...
public class HttpPartitionTest extends AbstractFullDistribZkTestBase {
  ...
  protected void testLeaderZkSessionLoss() throws Exception {
    ...
    try (SolrClient leaderSolr = getHttp2SolrClient(leader, testCollectionName)) {

      leaderSolr.add(doc);
      leaderSolr.close();

      // if the add worked, then the doc must exist on the new leader
      try (Http2SolrClient newLeaderSolr = getHttp2SolrClient(currentLeader, testCollectionName)) {
        assertDocExists(newLeaderSolr, testCollectionName, "2");
      }

    } catch (SolrException exc) {
      // this is ok provided the doc doesn't exist on the current leader
      try (SolrClient client = getHttp2SolrClient(currentLeader, testCollectionName)) {
  ...
  protected void assertDocNotExists(Http2SolrClient solr, String coll, String docId)
      throws Exception {
    NamedList<?> rsp = realTimeGetDocId(solr, docId);
  ...
  private NamedList<Object> realTimeGetDocId(Http2SolrClient solr, String docId)
