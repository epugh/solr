...
/** Unit tests for {@link MigrateDocs} */
public class MigrateDocsAPITest extends SolrTestCaseJ4 {

  private MigrateDocs api;
  ...
  public void setUp() throws Exception {
    ...
    api = new MigrateDocs(mockCoreContainer, realRequest, queryResponse);
  }
  ...
  public void testReportsErrorIfCollectionNameMissing() {
    final SolrException thrown =
        expectThrows(
            SolrException.class, () -> api.migrateDocs(null, new MigrateDocsRequestBody()));
  ...
  public void testAllParamsPassedCorrectly() throws Exception {
    final var requestBody = new MigrateDocsRequestBody();
    requestBody.target = "targetColl";
    requestBody.splitKey = "mySplitKey";
    requestBody.forwardTimeout = 1800;
    requestBody.followAliases = true;
    requestBody.async = "asyncId";

    api.migrateDocs("collName", requestBody);
    ...
    assertEquals("migrate", params.get(ACTION));
    assertEquals("collName", params.get("collection"));
    assertEquals("targetColl", params.get("target.collection"));
    assertEquals("mySplitKey", params.get("split.key"));
    assertEquals("1800", params.get("forward.timeout"));
