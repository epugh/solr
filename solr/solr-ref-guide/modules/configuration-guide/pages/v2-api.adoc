...
Example of introspect for a POST API that uses command-based dispatch: `\http://localhost:8983/api/cluster/plugin/_introspect?method=POST&command=add`
  ...
  "spec":[{
      ...
      "commands":{"add":{
          ...
          "properties":{
            ...
            "class":{
              "type":"string",
              "description":"The implementation class of the plugin. If the class is in a package, use the syntax 'package:class'."}}}}}],
  "WARNING":"This response format is experimental.  It is likely to change in the future."
}
----
...
For the "gettingstarted" collection, set the replication factor using the new JAX-RS path:

[source,bash]
----
$ curl -X POST http://localhost:8983/api/collections/gettingstarted/modify -H 'Content-type:application/json' -d '
{ "replicationFactor": "3" }'
