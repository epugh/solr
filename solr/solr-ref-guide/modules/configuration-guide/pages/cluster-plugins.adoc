...
Internally, plugin configurations are maintained in ZooKeeper in the `/clusterprops.json` file, under the `plugin` entry.
...
==== org.apache.solr.cluster.events.impl.AutoReplicaEventListener

An implementation of listener that reacts to NODES_UP events and automatically creates replicas on newly added nodes.

When nodes are added to the cluster, this listener will automatically create replicas for configured collections on those new nodes, distributing load across the cluster.
...
[%autowidth,frame=none]
|===
|Optional |Default: all collections
|===
+
Comma-separated list of collection names to manage.
If not specified or empty, all collections will be managed.
...
|===
+
Number of replicas to create per shard on new nodes.

Example plugin configuration to manage all collections:
...
Example plugin configuration to manage only specific collections:
