...
+
Single-node installations and user-managed clusters do not manage collections as first-class entities; instead they work directly with individual <<core,cores>>.
+
[[defcommit]]Commit::
...
In most contexts, thinking of "core" as synonymous with "replica" will clarify discussions about Solr's architecture.
+
...
[[solrclouddef]]xref:cluster-types.adoc#solrcloud-mode[SolrCloud]::
...
This includes both single-node installations and <<usermanaged,User-Managed>> clusters.
In source code and documentation, "Standalone" may refer to either <<usermanaged,User-Managed Mode>> or single-node deployments.
...
[[usermanaged]]xref:cluster-types.adoc#user-managed-mode[User-Managed Mode]::
